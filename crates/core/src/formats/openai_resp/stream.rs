    ...
    match event_type {
        ...
        "response.completed" => {
            ...
            let status = response
                ...
                .and_then(|v| v.as_str())
                .unwrap_or("completed");
            let stop_reason = helpers::status_to_stop_reason(status);
    ...
    match event {
        ...
        CanonicalStreamEvent::TextDelta(text) => {
            ...
            if state.accumulated_text.is_empty() {
                ...
                events.push(format_sse_event("response.output_item.added", &add_item.to_string()));
                ...
                events.push(format_sse_event("response.content_part.added", &add_part.to_string()));
            }
            ...
            events.push(format_sse_event("response.output_text.delta", &data.to_string()));
        ...
        CanonicalStreamEvent::Done { stop_reason, usage } => {
            let status = helpers::stop_reason_to_status(stop_reason);
            ...
            if !state.accumulated_text.is_empty() {
                ...
                events.push(format_sse_event("response.output_text.done", &text_done.to_string()));
                ...
                events.push(format_sse_event("response.content_part.done", &part_done.to_string()));
                ...
                events.push(format_sse_event("response.output_item.done", &item_done.to_string()));
            ...
            for tc in &state.accumulated_tool_calls {
                ...
                events.push(format_sse_event("response.function_call_arguments.done", &args_done.to_string()));
                ...
                events.push(format_sse_event("response.output_item.done", &fc_done.to_string()));
            ...
            events.push(format_sse_event("response.completed", &completed.to_string()));
...
data: {"type":"response.completed","response":{"id":"resp_123","status":"completed","usage":{"input_tokens":10,"output_tokens":5,"total_tokens":15}}}"#,
    ...
    fn test_emit_done_with_tool_calls_emits_output_item_done() {
        ...
        assert!(chunks.len() >= 3, "Expected at least 3 events, got {}", chunks.len());

        let has_args_done = chunks.iter().any(|c| c.contains("response.function_call_arguments.done"));
        assert!(has_args_done, "Missing response.function_call_arguments.done");

        let has_item_done = chunks.iter().any(|c| {
            c.contains("response.output_item.done") && c.contains("function_call")
        });
        assert!(has_item_done, "Missing response.output_item.done for function_call");

        let has_completed = chunks.iter().any(|c| c.contains("response.completed"));
        assert!(has_completed, "Missing response.completed");

        let completed_chunk = chunks.iter().find(|c| c.contains("response.completed")).unwrap();
        assert!(completed_chunk.contains("function_call"), "response.completed output missing function_call");
        assert!(completed_chunk.contains(r#""id":"fc_call_123""#), "response.completed missing item id");
        assert!(completed_chunk.contains(r#""call_id":"call_123""#), "response.completed missing call_id");
        assert!(completed_chunk.contains("shell"), "response.completed missing function name");
        assert!(completed_chunk.contains(r#"ls -la"#), "response.completed missing arguments");
    }
    ...
    fn test_emit_done_with_text_and_tool_calls() {
        ...
        assert!(chunks.len() >= 6, "Expected at least 6 events, got {}: {:?}", chunks.len(), chunks);

        let msg_done = chunks.iter().any(|c| {
            c.contains("response.output_item.done") && c.contains("message")
        });
        assert!(msg_done, "Missing response.output_item.done for message");

        let fc_done = chunks.iter().any(|c| {
            c.contains("response.output_item.done") && c.contains("function_call")
        });
        assert!(fc_done, "Missing response.output_item.done for function_call");

        let completed_chunk = chunks.iter().find(|c| c.contains("response.completed")).unwrap();
        assert!(completed_chunk.contains("message"), "completed missing message");
        assert!(completed_chunk.contains("function_call"), "completed missing function_call");
        assert!(completed_chunk.contains(r#""id":"fc_call_456""#), "completed missing item id");
        assert!(completed_chunk.contains("I'll explore the repo."), "completed missing text");
        assert!(completed_chunk.contains("read_file"), "completed missing function name");
    }
    ...
    fn test_full_tool_call_streaming_flow() {
        ...
        let start_chunks = OpenAIResponsesStreamAdapter::emit_sse_event(
            &CanonicalStreamEvent::Start { id: "resp_001".into(), model: "gpt-4.1".into() },
            &mut state,
        ).unwrap();
        assert_eq!(start_chunks.len(), 2, "Start should emit response.created + response.in_progress");
        ...
        assert!(tc_start[0].contains(r#""id":"fc_call_abc""#), "output_item.added missing item id");
        assert!(tc_start[0].contains(r#""status":"in_progress""#), "output_item.added missing status");
        ...
        assert!(delta1[0].contains(r#""item_id":"fc_call_abc""#), "delta missing item_id");
        ...
        let done_chunks = OpenAIResponsesStreamAdapter::emit_sse_event(
            &CanonicalStreamEvent::Done {
                stop_reason: StopReason::ToolUse,
                usage: Some(Usage { input_tokens: 50, output_tokens: 20, ..Default::default() }),
            },
            &mut state,
        ).unwrap();
        ...
        assert_eq!(state.accumulated_tool_calls.len(), 1);
        assert_eq!(state.accumulated_tool_calls[0].arguments, r#"{"command":"ls"}"#);

        let fc_item_done = done_chunks.iter().find(|c| {
            c.contains("response.output_item.done") && c.contains("function_call")
        });
        assert!(fc_item_done.is_some(), "Missing output_item.done for function_call");

        let fc_str = fc_item_done.unwrap();
        assert!(fc_str.contains(r#""id":"fc_call_abc""#), "output_item.done missing item id");
        assert!(fc_str.contains(r#""call_id":"call_abc""#), "output_item.done missing call_id");
        assert!(fc_str.contains("shell"), "output_item.done missing function name");
        assert!(fc_str.contains("command") && fc_str.contains("ls"), "output_item.done missing arguments");
    }
}
